...
public class SearchTextField extends JTextField {
    ...
    private final List<OpsiSearch> items = new ArrayList<>();
    private final List<EventOpsiSearch> events = new ArrayList<>();
    ...
    public void addOption(OpsiSearch option) {
        items.add(option);
        if (selectedIndex == -1) {
            selectedIndex = 0;
            runEvent();
    ...
    public void addEventOptionSelected(EventOpsiSearch event) {
        events.add(event);
    }

    public OpsiSearch getSelectedOption() {
        if (selectedIndex == -1) {
            return null;
        } else {
    ...
    private void runEvent() {
        for (EventOpsiSearch event : events) {
